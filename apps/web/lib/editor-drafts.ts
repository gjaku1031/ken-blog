...
export type DraftDetail = {
  ...
  visibility: "PUBLIC" | "PRIVATE"; createdAt: string; updatedAt: string; attachmentIds: number[];
};
/** 목록 SQL이 본문 열을 읽지 않는 편집본 한 행. */
export type DraftSummary = Omit<DraftDetail, "body" | "attachmentIds">;
...
export type AdminPost = { id: number; title: string; slug: string; body: string; updatedAt: string;
  ...
  category: { id: number } | null; tags: string[]; attachmentIds: number[] };
/** 전체 교체 저장과 신규 편집본 생성에서 공통으로 보내는 값. */
export type DraftValues = Pick<DraftDetail, "title" | "slug" | "body" | "categoryId" | "tags" | "visibility" | "attachmentIds">;
...
export function parseAdminPost(value: unknown): AdminPost {
  ...
  return { id: integer(item.id, 1), title: item.title, slug: item.slug, body: item.body,
    ...
    attachmentIds: attachmentIds(item.attachmentIds) };
}
