...
/** raw HTML과 이미지 자동 요청을 차단한 GFM 읽기 전용 렌더러. */
export function SafeMarkdown({ body }: { body: string }) {
  return <div className="markdown-body"><Markdown remarkPlugins={[remarkGfm]} skipHtml urlTransform={safeUrl}
    components={{
      ...
      /** 긴 코드 블록에 키보드 초점을 허용해 내부 가로 스크롤을 사용할 수 있게 한다. */
      pre({ node: _node, ...props }) { return <pre {...props} tabIndex={0} role="region" aria-label="코드 블록, 가로로 스크롤 가능" />; },
