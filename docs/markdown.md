...
## P2-06A 위키 제목 조회 — main 반영 완료

`[[글 제목]]`의 대상 글을 제목으로 찾는 API 단계. P2-06A 당시 글 본문을 파싱하거나 링크로 표시하지 않았고 관리자 편집기 선택기·삽입 버튼도 없었음. 읽기 화면의 위키 모양 텍스트는 당시 원문으로 남았으며, 화면 연결은 P2-06B에서 진행. Tech의 출간 글과 Projects/Notes를 섞어 조회하지 않으며 역링크·목차도 아직 없음. 2026-09-26 기존 API 검사 28개와 격리 HTTP에서 반복 제목·쉼표·Unicode·권한별 상태·입력 상한·CORS/no-store 확인. DB 장애 503과 복구 후 기존 세션 유지, 최종 JAR의 OpenAPI·응답 재검증 및 소유 API·MySQL 컨테이너/볼륨 정리 완료. Swagger 보완 뒤 최종 패키징은 테스트 실행 없이 수행. main `d5638dd`·[CI](https://github.com/gjaku1031/ken-blog/actions/runs/36239305385)·[Pages](https://github.com/gjaku1031/ken-blog/actions/runs/36239305344) 반영 완료, 실제 push는 2026-09-26 11:35:32 UTC. 제목 조회의 권한·입력·상태 계약은 [위키 제목 조회 API](wiki-links.md) 참고. 공개 HTTPS API 연결과 기존 운영 배포 변경 없음.

## P2-06B 본문·주석 위키 링크 — main 반영 완료

읽기 화면의 명확한 `[[제목]]`과 `[[제목|표시명]]`을 제목 조회 API에 연결하는 범위. 본문·표·안전한 접기와 주석 내용은 대상이며, 코드·수식·도식·이미지·기존 Markdown 링크·raw HTML의 표기는 위키 링크로 해석하지 않음. 잘못된 위키 문법도 저장 원문은 보존하되 화면에서는 기존 Markdown 표시·HTML 차단 규칙을 따름. 2026-09-26 기존 웹 검사 7개·타입 검사·API 설정 정적 빌드, 격리 브라우저의 링크 이동·잠금/없음/미확인 구분·재시도·로그아웃 뒤 정보 폐기를 확인. 편집 원문/주석 번호 보존과 390/1280px 양 테마 가로 넘침 0, 검사한 두 화면 Axe 위반 0건 확인. 상세 문법과 검증 범위는 [위키 링크 계약](wiki-links.md) 참고. 검증 자원 정리 후 main `a6f8a6e`·[CI](https://github.com/gjaku1031/ken-blog/actions/runs/36240322404)·[Pages](https://github.com/gjaku1031/ken-blog/actions/runs/36240322370) 반영 완료. 공개 HTTPS API 연결 없음.

P2-06C의 관리자 제목 선택기·읽기 역링크·h2/h3 목차는 브라우저 통합 검증 중. 저장 본문의 명시적 `wikiTargets` 목록으로 역링크를 계산하므로 예전 글은 새 편집기로 재저장하거나 [격리 재색인 도구](../ops/wiki-links/README.md)로 보정하기 전까지 역링크에서 빠질 수 있음. 기존 API 검사 28개와 격리 V9→V10·검색·보정·역링크의 권한/SQL 계약은 확인. 자세한 근거는 [위키 링크 안내](wiki-links.md) 참고. 실제 브라우저 검증·main 반영은 아직 완료로 기록하지 않음.
