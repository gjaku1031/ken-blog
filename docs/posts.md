...
P1-04A의 관리자 API는 MySQL 게시글 **초안**의 작성·목록·상세·전체 수정·삭제 제공. P1-04B의 출간/철회·공개 범위와 권한별 읽기 API는 2026-09-26 격리 검증 후 main·CI·Pages 반영 완료. 실제 근거는 [계획](../planning/issues/P1-04B.md)에 기록. P1-04의 익명 PUBLIC 상세 본문 캐시도 격리 JAR·Buildpacks/Compose 검증 후 main·CI·Pages 반영 완료. P1-05A의 Tech 분류·태그·필터 API는 기존 검사 28개와 격리 JAR HTTP·SQL·공개 본문 캐시 활성 호환 검증 후 main·CI·Pages 반영 완료. P2-01의 브라우저 공개 Tech 화면도 main·CI·Pages 반영 완료했으나 공개 API 주소는 여전히 미설정. 브라우저 관리자 편집 화면과 게시글-첨부 연결은 아직 없음. 관리자 요청은 격리 로컬 API의 HTTP 도구나 Swagger에서 사용. 별도 수동 저장 편집본은 [P2-02A 계약](editor-drafts.md)이며 격리 검증 완료·main 반영 전.
...
기존 게시글 PUT에는 수정 버전이나 낙관적 잠금이 없음. 두 관리자가 같은 글을 동시에 수정하면 **나중에 커밋된 본문 값이 최종 값이 될 수 있음**. P1-04B의 출간/철회/공개 범위 변경과 PUT은 같은 행에 쓰기 잠금을 걸어 오래 읽은 상태 필드로 다른 변경을 덮지 않도록 구성. P2-02A는 별도 편집본에 `revision`과 원본 기준 `updatedAt` 충돌 검사를 추가했으며 기존 PUT 계약을 소급 변경하지 않음. 자동 저장·협업 편집 보장 없음. DELETE는 게시글 행과 V8의 연결 편집본을 함께 제거하지만 아직 관계가 없는 OCI Object Storage 객체나 첨부 메타데이터를 함께 삭제하지 않음. 게시글에 이미지를 연결하는 기능과 정리 정책은 후속 계획 대상.
