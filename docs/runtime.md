...
## P2-03A 이미지 연결 실행 경계 — 격리 검증·main 반영 완료

P2-03A는 기존 API·MySQL Compose와 비공개 OCI Object Storage 설정을 사용하고 Flyway V9에 글·편집본 이미지 연결 관계를 추가. 별도 저장소 서비스·웹 컨테이너·Redis 이미지 캐시·버킷 공개 정책·VM 자동 배포는 없음. 새 `/api/v1/posts/{postId}/attachments/{id}/content`는 글 ID와 이미지 ID를 받아 현재 MySQL에서 출간·범위·연결·READY를 확인한 뒤 Spring이 OCI 원본을 전달. 관리자 첨부 GET/POST/DELETE의 브라우저 CORS는 `APP_AUTH_CORS_ALLOWED_ORIGINS`에 명시한 정확한 인증 origin만 자격 증명 허용, 쓰기에는 기존 세션·CSRF 필요. 공개 전용 origin은 관리자 첨부 접근 대상이 아님. 기존 V8 데이터·JDBC 세션의 V9 전환, 실제 OCI PNG/JPEG 바이트·권한별 이미지 읽기, 연결/삭제 경합과 편집본 출간을 격리 HTTP·SQL로 확인. 격리 MySQL 중단 중 익명·인증 이미지 모두 `503 ProblemDetail`, 복구 뒤 기존 JDBC 세션·이미지 복원을 확인. 별도 새 DB의 V1~V9·관리자 준비·로그인·글 생성 HTTP도 확인. 검증 소유 글·편집본·첨부의 연결을 해제하고 첨부 DELETE `204`·OCI HEAD `404`·정확한 소유 접두사 목록 0·메타데이터 0을 확인. 소유 JAR·MySQL 컨테이너·볼륨 제거 후 기존 VM 앱·Redis의 ID·이미지·시작 시각 동일성 확인. main `5fd3e32`·[CI](https://github.com/gjaku1031/ken-blog/actions/runs/36230515666)·[Pages](https://github.com/gjaku1031/ken-blog/actions/runs/36230515657) 반영 완료.

로컬의 `http://127.0.0.1:14000`과 `http://127.0.0.1:18081`은 origin이 달라도 동일 host의 같은 site이므로 `SameSite=Lax` 세션 검증이 가능. 반면 GitHub Pages와 향후 API 도메인이 교차 site이면 이 로컬 결과만으로 쿠키가 PRIVATE 이미지 요청에 전송된다고 볼 수 없음. 공개 HTTPS API 도메인과 쿠키·CSRF·자격 증명 정책을 별도로 확정해야 하며 현재 Pages 빌드의 API URL은 비어 있음. 공개 이미지 블록과 관리자 업로드 UI는 P2-03B 격리 구현·검증 범위. 기존 VM 앱·Redis·버킷 정책은 이 계획으로 교체하지 않음.

## P2-03B 이미지 화면의 로컬 검증·자원 정리 완료

P2-03B는 API·DB·비공개 OCI Object Storage의 기존 토폴로지를 바꾸지 않고 정적 웹의 이미지 편집·읽기 경로를 연결하는 작업. 설정된 API 빌드에서만 관리자 세션/CSRF를 갖춘 multipart 업로드와 글 ID 기반 이미지 GET을 브라우저에서 검증. 로컬 정적 서버와 API를 모두 `127.0.0.1` 호스트로 맞추고 인증 origin CORS를 정확히 지정. 관리자 업로드·미리보기는 인증 origin과 쿠키·CSRF, 익명 PUBLIC 이미지는 허용된 공개 GET, PRIVATE 이미지는 유효한 USER/ADMIN 세션을 전제로 함. 브라우저는 OCI에 직접 접속하지 않음.

공개 Pages 빌드의 `NEXT_PUBLIC_API_BASE_URL`은 여전히 비어 있음. HTTPS Pages에서 로컬 HTTP API를 사용하거나 로컬 같은 site 쿠키 검증을 교차 site 운영 로그인·비공개 이미지 검증으로 대체할 수 없음. 현재 단계에는 버킷 공개, CDN, 이미지 바이트 영구 캐시, 자동 미참조 객체 삭제, 기존 VM 앱 교체 없음. 2026-09-26 현재 P2-03B의 API 설정 정적 빌드·타입 검사·기존 웹 검사 7개와 격리 브라우저의 실제 OCI 업로드/저장/출간/PUBLIC·PRIVATE 읽기를 확인. 클립보드는 CDP 격리 권한으로 실제 Ctrl+V, 파일 드롭은 합성 DataTransfer 이벤트로 확인했으며 물리적 드롭과 일반 운영체제 클립보드 검증은 아님. 주입한 업로드·이미지 `503`과 실제 CSRF `403`에서 현재 입력 유지/명시적 재시도, 지연 업로드 취소·경로 변경의 다른 원고 삽입 방지, 세션 만료의 브라우저 원고 제거와 서버 편집본 복원을 확인. 일곱 너비 라이트·다크에서 페이지 넘침 0, 검사한 네 화면 Axe 위반 0건·미처리 오류 0건. API 미설정 별도 정적 빌드의 Home·글쓰기·글 상세는 API 요청 0건·가짜 이미지 0건. 검증 소유 글·편집본·연결·첨부를 정리하고 모든 OCI key HEAD `404`·정확한 소유 접두사 비움을 확인. 전용 API·MySQL·볼륨 제거, 기존 앱·Redis ID·이미지·시작 시각 불변. main 반영 전. 사용자 흐름은 [관리자 글쓰기](editor.md), 이미지 표기와 안전 경계는 [Markdown 읽기](markdown.md), 소유 객체 점검은 [첨부 운영](attachments.md) 참고.
